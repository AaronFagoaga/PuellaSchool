...
class UserModel
{
    ...
    public function authenticate($userNickname, $userPassword)
    {
        $query = "SELECT * FROM tbl_user WHERE userNickname = :nickname AND userPassword = :password";

        $result = $this->conn->prepare($query);

        $userNickname = htmlspecialchars(strip_tags($userNickname));
        $userPassword = md5(htmlspecialchars(strip_tags($userPassword)));

        $result->bindParam(":nickname", $userNickname);
